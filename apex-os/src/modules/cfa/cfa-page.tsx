...
export default function CFAPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto text-foreground font-sans">
      {/* Top Banner Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-r from-blue-950/40 via-red-950/20 to-transparent p-6 rounded-[20px] border border-blue-500/20 relative overflow-hidden">
        ...
        {/* Countdown & Pace Card */}
        <motion.div 
          className="bg-surface-1 border border-blue-500/30 p-5 rounded-2xl flex items-center gap-4 shrink-0 z-10"
      ...
      <Card className="bg-surface-1 border-blue-500/20 rounded-[20px] p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6">
          ...
          <Button 
            ...
            size="sm" 
            className="bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs gap-1.5"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 justify-items-stretch">
          {stats?.modules.map((m) => {
            ...
            if (m.percentage >= 75) ringColor = '#22C55E'; // Green
            else if (m.percentage >= 25) ringColor = '#F5A524'; // Amber
            else if (m.percentage > 0) ringColor = '#3B6EF6'; // Blue
            ...
            return (
              <motion.div 
                ...
                className="flex flex-col items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-line hover:border-blue-500/50 hover:bg-blue-500/10 transition-all cursor-pointer group text-center relative overflow-hidden"
              >
                <div className="w-full flex items-center justify-between mb-2 text-[11px] font-bold">
                  <span className="text-blue-400 font-mono">{m.name}</span>
                  <span className="px-2 py-0.5 rounded-full bg-white/10 text-zinc-300 border border-line/70">
                    {m.short}
                  </span>
                </div>
                ...
                <div className="w-full mt-2 pt-2 border-t border-line/70">
                ...
                <div className="text-[11px] text-blue-400 font-semibold md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100 transition-opacity mt-1.5 flex items-center gap-1">
                  Open Module →
                </div>
              </motion.div>
            );
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card className="w-full bg-surface-1 border-blue-500/20 rounded-[20px] p-6">
          ...
          <div className="h-[340px] pt-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={moduleProgressData} layout="vertical" margin={{ top: 5, right: 30, left: 35, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1A2030" opacity={0.4} horizontal={false} />
                <XAxis type="number" domain={[0, 100]} stroke="#8A93A6" fontSize={11} tickFormatter={(v) => `${v}%`} />
                <YAxis dataKey="name" type="category" stroke="#8A93A6" fontSize={11} tickLine={false} axisLine={false} />
                <Tooltip content={<CustomBarTooltip />} />
                <Bar dataKey="userProgress" name="Completion %" radius={[0, 6, 6, 0]}>
                  {moduleProgressData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={entry.userProgress >= 75 ? '#22C55E' : entry.userProgress >= 25 ? '#3B6EF6' : '#ef4444'} 
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <div className="h-[340px] pt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats?.trajectory || []}>
                ...
                <XAxis dataKey="month" stroke="#8A93A6" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="#8A93A6" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${v}h`} />
                <Tooltip 
                ...
                <Area type="monotone" dataKey="targetHours" name="Target (hrs)" stroke="#3B6EF6" strokeWidth={2} strokeDasharray="4 4" fill="url(#targetHoursGrad)" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link 
          to="/cfa/topics" 
          className="p-6 rounded-[20px] bg-gradient-to-br from-blue-900/30 via-surface-1 to-void border border-blue-500/30 hover:border-blue-500/60 transition-all group flex items-center justify-between"
        >
        ...
        <Link 
          to="/cfa/revision" 
          className="p-6 rounded-[20px] bg-gradient-to-br from-red-900/30 via-surface-1 to-void border border-red-500/30 hover:border-red-500/60 transition-all group flex items-center justify-between"
        >
          <div className="space-y-1">
