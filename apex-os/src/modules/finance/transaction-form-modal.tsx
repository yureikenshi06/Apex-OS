...
export default function TransactionFormModal({ isOpen, onClose, transaction }: TransactionFormModalProps) {
  ...
  const typeColors: Record<string, string> = {
    'Expense': 'bg-rose-600/90 text-white',
    'Income': 'bg-emerald-600/90 text-white',
    'Transfer': 'bg-blue-600/90 text-white',
  };

  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="bg-surface-1 border-blue-500/30 text-white max-w-2xl max-h-[90vh] overflow-y-auto rounded-[20px]">
          <DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-2">
              ...
              {(['Expense', 'Income', 'Transfer'] as const).map((t) => (
                <button
                  ...
                  className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${
                    txType === t ? typeColors[t] + '' : 'text-zinc-400 hover:text-white hover:bg-white/5'
                  }`}
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input type="date" {...register('date')} className="bg-surface-2 border-line text-white rounded-xl" required />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-300 font-bold uppercase tracking-wider">Amount (₹)</Label>
                <Input type="number" step="0.01" {...register('amount')} className="bg-surface-2 border-line text-white rounded-xl text-lg font-mono font-bold" placeholder="0.00" required />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input {...register('description')} className="bg-surface-2 border-line text-white rounded-xl" placeholder="What was this for?" required />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-300 font-bold uppercase tracking-wider">Merchant / Payee</Label>
                <Input {...register('merchant_payee')} className="bg-surface-2 border-line text-white rounded-xl" placeholder="e.g. Swiggy, Amazon" />
              </div>
            </div>
                ...
                {isQuickAddingSubcat ? (
                  <div className="flex gap-1.5">
                    <Input
                      ...
                      className="bg-surface-2 border-blue-500/40 text-white rounded-xl h-9 text-xs"
                      autoFocus
                    />
                    <Button
                      type="button"
            ...
            {/* Recurring toggle */}
            <div className="flex items-center justify-between p-4 bg-white/[0.03] rounded-xl border border-line">
              <div>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea {...register('notes')} className="bg-surface-2 border-line text-white rounded-xl min-h-[70px]" placeholder="Optional notes..." />
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-line">
              ...
              <Button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold px-6" disabled={addMutation.isPending || updateMutation.isPending}>
