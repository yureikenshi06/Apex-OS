...
export default function SplitsPage() {
  ...
  const SplitCard = ({ split, color }: { split: any, color: string }) => (
    <motion.div
      ...
      className={`bg-surface-1 border border-${color}-500/20 rounded-2xl p-4 group hover:border-${color}-500/40 transition-colors`}
    >
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-line/70">
        ...
        <button onClick={() => handleOpenEdit(split)} className="p-1.5 rounded-lg hover:bg-blue-500/20 text-zinc-400 hover:text-blue-400 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100 transition-all">
          <Edit2 className="w-3.5 h-3.5" />
        </button>
        <button onClick={() => handleDelete(split.id)} className="p-1.5 rounded-lg hover:bg-red-500/20 text-zinc-400 hover:text-red-400 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100 transition-all">
    ...
    <div className="space-y-6 max-w-7xl mx-auto text-foreground">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[22px] font-extrabold tracking-tight md:text-3xl">Splits & Debts</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-surface-1 border border-emerald-500/20 rounded-2xl p-5">
          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1"><ArrowUpRight className="w-3 h-3 text-emerald-400" /> Owed to Me</span>
        ...
        <div className="bg-surface-1 border border-rose-500/20 rounded-2xl p-5">
          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1"><ArrowDownRight className="w-3 h-3 text-rose-400" /> I Owe</span>
        ...
        <div className="bg-surface-1 border border-blue-500/20 rounded-2xl p-5">
          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Net Balance</span>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          {owedToMe.length === 0 ? (
            <p className="text-zinc-500 text-sm text-center py-8 bg-white/[0.02] rounded-2xl border border-line/70">No one owes you anything right now.</p>
          ) : (
            <div className="space-y-3">
        ...
        <div className="space-y-3">
          ...
          {iOwe.length === 0 ? (
            <p className="text-zinc-500 text-sm text-center py-8 bg-white/[0.02] rounded-2xl border border-line/70">You don't owe anything right now.</p>
          ) : (
            <div className="space-y-3">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {settled.slice(0, 6).map((s: any) => (
              <div key={s.id} className="bg-white/[0.02] border border-line/70 rounded-xl p-3 flex justify-between items-center">
                <div>
                  <span className="text-sm font-medium text-zinc-400">{s.person_name}</span>
                  <span className="text-xs text-zinc-500 block">{s.description}</span>
                </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-md bg-surface-1 border-blue-500/30 text-white rounded-[20px]">
          <DialogHeader>
          ...
          <form onSubmit={handleSave} className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input value={personName} onChange={(e) => setPersonName(e.target.value)} placeholder="e.g. Rahul, Priya" required className="bg-surface-2 border-line text-white rounded-xl" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input type="number" step="1" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="500" required className="bg-surface-2 border-line text-white rounded-xl" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-300 font-bold uppercase tracking-wider">Description</Label>
                <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g. Dinner split" className="bg-surface-2 border-line text-white rounded-xl" />
              </div>
            </div>
            ...
            <DialogFooter className="pt-3 border-t border-line flex justify-end gap-2">
              ...
              <Button type="submit" disabled={addSplit.isPending || updateSplit.isPending} className="bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold px-6">
