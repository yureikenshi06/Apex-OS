    ...
    <div className="space-y-6 max-w-7xl mx-auto text-foreground">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[22px] font-extrabold tracking-tight md:text-3xl">Budget Limits</h1>
        </div>
          ...
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                <XAxis dataKey="category" stroke="#8A93A6" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="#8A93A6" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `₹${v/1000}k`} />
                <RechartsTooltip 
                  contentStyle={{ backgroundColor: '#12161F', borderColor: '#1A2030', borderRadius: '12px', color: '#fff' }} 
                  formatter={(val: any) => [`₹${Number(val).toLocaleString()}`, '']}
                />
                <Bar dataKey="budget" name="Budget" fill="#3B6EF6" radius={[4, 4, 0, 0]} />
                <Bar dataKey="actual" name="Actual" radius={[4, 4, 0, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.actual > entry.budget ? '#ef4444' : '#22C55E'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <AnimatePresence>
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-16 text-zinc-500">
              <Target className="w-10 h-10 mx-auto mb-3 text-zinc-500" />
            ...
            budgets.map((budget: any, idx: number) => {
              ...
              return (
                <motion.div 
                  ...
                  className="bg-surface-1 border border-line hover:border-blue-500/30 rounded-2xl p-5 transition-colors group"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex items-center gap-1.5 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100 transition-opacity">
          ...
          <form onSubmit={handleSave} className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                ...
                required
                className="bg-surface-2 border-line text-white rounded-xl text-lg font-mono"
              />
            </div>
            ...
            <DialogFooter className="pt-3 border-t border-line flex justify-end gap-2">
              ...
              <Button type="submit" disabled={addBudget.isPending || updateBudget.isPending} className="bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold px-6">
