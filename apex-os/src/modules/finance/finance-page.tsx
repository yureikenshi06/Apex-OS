...
const SpikeDayTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className={`bg-[#0e131f]/95 border p-3 rounded-2xl shadow-2xl backdrop-blur-md min-w-[130px] ${isSpike ? 'border-rose-500/40' : 'border-blue-500/20'}`}>
      ...
      {isSpike && <p className="text-[10px] text-rose-400 font-bold mt-1">⚡ Impulse Spike</p>}
    </div>
  );
};
...
const DowTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-[#0e131f]/95 border border-blue-500/20 p-3 rounded-2xl shadow-2xl backdrop-blur-md">
...
const TrendTooltip = ({ active, payload, label }: any) => {
  if (!active || !payload?.length) return null;
  return (
    <div className="bg-[#0e131f]/95 border border-blue-500/20 p-3.5 rounded-2xl shadow-2xl backdrop-blur-md min-w-[160px]">
      <p className="text-xs font-bold text-white mb-2 pb-1.5 border-b border-white/10">{label}</p>
...
const CumulativeTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-[#0e131f]/95 border border-indigo-500/30 p-3.5 rounded-2xl shadow-2xl backdrop-blur-md min-w-[160px]">
      ...
      <div className="space-y-1.5 text-xs">
        ...
        <div className={`flex justify-between gap-4 pt-1 border-t border-white/10 font-bold ${diff > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
...
function SpendingInsightsPanel({ filter }: { filter?: FinanceAnalysisFilter }) {
  ...
  if (isLoading) return (
    <div className="space-y-4 animate-pulse">
      ...
      <div className="h-52 bg-white/[0.02] rounded-3xl border border-white/5" />
  ...
  if (ins && !ins.hasData) {
    return (
      <Card className="bg-[#0b0f19]/80 border border-white/10 rounded-3xl p-8 text-center">
  ...
  const NVW_COLORS       = ['#10b981', '#f59e0b'];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {timeframeLabel && (
                <Badge variant="outline" className="text-[10px] border-rose-500/30 text-rose-300 bg-rose-500/10 font-bold px-2 py-0.5">
                  {timeframeLabel}
                </Badge>
              )}
            </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {spikeDays > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-rose-500/10 border border-rose-500/25 text-xs">
              ...
              <span className="text-rose-500 font-mono text-[10px]">· ₹{totalImpulseSpend.toLocaleString()} burned</span>
            </div>
          )}
      ...
      <Card className="bg-[#0b0f19]/80 border-rose-500/15 backdrop-blur-xl rounded-3xl shadow-2xl p-5 min-w-0">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-4">
          ...
          {spikeDays > 0 && (
            <div className="shrink-0 text-right">
              ...
              <div className="text-[10px] text-zinc-600">across {spikeDays} spike {timelineUnit}{spikeDays > 1 ? 's' : ''}</div>
            </div>
          )}
        </div>
        <div className="h-[200px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={dailySpikeData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }} barCategoryGap="20%">
              ...
              <XAxis dataKey="label" stroke="#475569" fontSize={10} tickLine={false} axisLine={false} interval={dailySpikeData.length > 15 ? 4 : 1} />
              <YAxis stroke="#475569" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(v) => v === 0 ? '' : `₹${v >= 1000 ? `${(v/1000).toFixed(0)}k` : v}`} />
              ...
              <Bar dataKey="amount" radius={[3,3,0,0]} maxBarSize={20}>
                {dailySpikeData.map((entry, i) => (
                  <Cell key={i} fill={entry.isSpike ? '#ef4444' : entry.amount > 0 ? '#3b82f6' : '#1e293b'} fillOpacity={entry.isSpike ? 1 : 0.7} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="flex items-center gap-4 mt-3 justify-center text-xs text-zinc-500">
          ...
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#1e293b]" /> No spend</span>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Card className="bg-[#0b0f19]/80 border-blue-500/15 backdrop-blur-xl rounded-3xl shadow-2xl p-5 min-w-0">
          <div className="mb-4">
          ...
          <div className="h-[220px] w-full">
            {topCats.length === 0 ? (
              <div className="h-full flex items-center justify-center text-xs text-zinc-600">Not enough history yet</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
          ...
          {categoryGrowth.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-4 pt-3 border-t border-white/5">
              {categoryGrowth.map((cg, i) => {
                const color = COLORS[i % COLORS.length];
                return (
                  <div key={cg.category} className="p-2.5 rounded-xl bg-white/[0.02] border border-white/5 flex flex-col justify-between">
                    ...
                    <div className="text-xs font-mono font-bold text-white">
                      ...
                      <span className="text-[10px] font-normal text-zinc-500 ml-1">vs ₹{cg.previous.toLocaleString()}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <Card className="bg-[#0b0f19]/80 border-amber-500/15 backdrop-blur-xl rounded-3xl shadow-2xl p-5 min-w-0">
          <div className="mb-4">
          ...
          <div className="h-[220px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dayOfWeekData} layout="vertical" margin={{ top: 4, right: 40, left: 8, bottom: 4 }} barCategoryGap="18%">
                <XAxis type="number" stroke="#475569" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(v) => v === 0 ? '' : `₹${v >= 1000 ? `${(v/1000).toFixed(0)}k` : v}`} />
                <YAxis type="category" dataKey="day" stroke="#94a3b8" fontSize={11} tickLine={false} axisLine={false} width={28} />
                ...
                <Bar dataKey="avgSpend" name="Avg spend" radius={[0,4,4,0]} maxBarSize={18}>
                  {dayOfWeekData.map((entry, i) => {
                    ...
                    return (
                      <Cell key={i}
                        fill={entry.day === peakDay ? '#f59e0b' : `rgba(59,130,246,${0.3 + intensity * 0.7})`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card className="bg-[#0b0f19]/80 border-emerald-500/15 backdrop-blur-xl rounded-3xl shadow-2xl p-5 min-w-0">
          <div className="mb-3">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 w-full space-y-3">
              <div className="p-3 rounded-2xl bg-emerald-500/8 border border-emerald-500/20">
                <div className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider mb-0.5">Needs</div>
              ...
              <div className="p-3 rounded-2xl bg-amber-500/8 border border-amber-500/20">
                <div className="text-[10px] text-amber-400 font-bold uppercase tracking-wider mb-0.5">Wants</div>
              ...
              {savingsPotential > 0 && (
                <div className="p-2.5 rounded-xl bg-blue-500/8 border border-blue-500/20">
                  <div className="text-[10px] text-blue-400 font-bold">💡 Savings Opportunity</div>
        ...
        <Card className="bg-[#0b0f19]/80 border-indigo-500/15 backdrop-blur-xl rounded-3xl shadow-2xl p-5 min-w-0">
          <div className="mb-4">
          ...
          <div className="h-[200px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={cumulativeData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" opacity={0.4} vertical={false} />
                <XAxis dataKey="day" stroke="#475569" fontSize={10} tickLine={false} axisLine={false} interval={cumulativeData.length > 15 ? 4 : 1} />
                <YAxis stroke="#475569" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(v) => v === 0 ? '' : `₹${v >= 1000 ? `${(v/1000).toFixed(0)}k` : v}`} />
                <RechartsTooltip content={<CumulativeTooltip />} />
                <Area type="monotone" dataKey="ideal" stroke="#475569" strokeWidth={1.5} strokeDasharray="4 4" fill="url(#idealGrad)" />
                <Area type="monotone" dataKey="actual" stroke="#6366f1" strokeWidth={2.5} fill="url(#actualGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
      ...
      {(impulseTransactions.length > 0 || impulseByCategory.length > 0) && (
        <Card className="bg-[#0b0f19]/80 border-rose-500/10 rounded-3xl p-5 shadow-2xl min-w-0">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-rose-400" /> Impulse Spending ({timeframeLabel || 'Selected Period'})
            </h3>
            <span className="text-[10px] text-zinc-600 font-mono hidden sm:block">discretionary · above avg transaction</span>
          </div>
          ...
          {impulseByCategory.length > 0 && (
            <div className="mb-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
              <div className="text-xs font-bold text-zinc-300 mb-3 flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-zinc-500">{impulseByCategory.length} categories</span>
              </div>
                ...
                {impulseByCategory.map(cat => (
                  <div key={cat.name} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono font-bold text-rose-300">₹{cat.total.toLocaleString()} <span className="text-zinc-500 text-[10px] font-normal">({cat.pct}%)</span></span>
                    </div>
                ...
                {impulseTransactions.map((tx: any, i: number) => (
                  <div key={tx.id || i} className="p-3.5 rounded-2xl bg-rose-500/5 border border-rose-500/15 hover:border-rose-500/30 transition-all">
                    <div className="flex items-center justify-between mb-1.5">
                      <Badge variant="outline" className="text-[9px] px-1.5 py-0 border-rose-500/20 text-rose-300 bg-rose-500/10">{tx.category}</Badge>
                      <span className="text-[10px] text-zinc-600 font-mono">{new Date(tx.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
                    </div>
                    ...
                    {i === 0 && <p className="text-[9px] text-rose-500 mt-1 font-bold">👑 Biggest impulse in this period</p>}
                  </div>
                ))}
              </div>
            </>
